...
Мой код для записи новой строки "Новая строка, которую я добавил" в "dev3"

"""


def __add_line(collection: List[str]) -> str:
    try:
        return collection.append("Новая строка, которую я добавил")
    except IndexError:
        raise IOError('Запись невозможна')


def write_line(device: Device) -> str:
    if not is_writable_device(device):
        raise PermissionError('Добавление новой строки')

    return __add_line(device.data)


device: Device = open_device('/devices/dev3')
write_line(device)

# Проверка, что новая строка добавлена
for i in range(4):
